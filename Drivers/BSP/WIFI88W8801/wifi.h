...
typedef  struct _MrvlIEtypes_eapol_pkt_t
{
    ...
    MrvlIEtypesHeader_t header;
    /** eapol pkt buf */
    uint8_t pkt_buf[0];//不会要被编译原理上一课吧
...
//可以看做是一个总命令就是有啥功能在这个下面写就行

void mrvl88w8801_get_control_io_port(void);

static uint8_t  mrvl88w8801_func_init(uint8_t* tx); //真正初始化固件的函数
static uint8_t mrvl88w8801_init_fw(void); //初始化固件

static uint8_t mrvl88w8801_get_hw_spec(uint8_t* tx);
static uint8_t mrvl88w8801_ret_get_hw_spec(uint8_t *rx_buffer,int len);

// 初始化的时候用的 准备mac - 得到mac  以及中断处理和得到端口号
...
uint8_t mrvl88w8801_process_packet(void); //核心函数


// 用于搜索和连接的函数
static uint8_t mrvl88w8801_scan_prepare(uint8_t* tx,void *data_buff,uint16_t data_len); //搜索前的准备
uint8_t mrvl88w8801_scan(uint8_t *channel,uint8_t channel_num,uint16_t max_time);
static uint8_t mrvl88w8801_ret_scan(uint8_t *rx_buffer,int len); //在cmd 里面调用的
uint8_t mrvl88w8801_scan_ssid(uint8_t *ssid,uint8_t ssid_len,uint16_t max_time); //这个我想是后面用的到的

// 用于连接的
static uint8_t mrvl88w8801_ass_supplicant_pmk_pkg(uint8_t *bssid); //建立连接的时候可能会用到的
...
uint8_t mrvl88w8801_connect(uint8_t *ssid,uint8_t ssid_len,uint8_t *pwd,uint8_t pwd_len);//连接到某个wifi上
static uint8_t mrvl88w8801_ret_connect(uint8_t *rx_buffer,int len);
static uint8_t mrvl88w8801_disconnect_prepare(uint8_t* tx);
uint8_t mrvl88w8801_disconnect(void); //断开连接

//----用于TCP层的连接的-----
static uint8_t mrvl88w8801_process_data(uint8_t *rx_buffer,int len);
uint8_t *mrvl88w8801_get_send_data_buf(void);//加入我们接受到的命令转发就用下面俩函数的组合
...
//第三方
/*-------------------------------------------------------------------*/
uint8_t wifi_start_scan(scan_condition_t *scan_condition); //开始扫描 这个就是应用向的了
uint8_t wifi_connect_ap(uint8_t *ssid,uint8_t ssid_len,uint8_t *pwd,uint8_t pwd_len);
uint8_t wifi_disconnect_ap(void); //断开连接


/*----------------------------*/
//为了方便在RTOS中操作 开启了一个中断
void sd_irq_exit_enable(void);
void rtos_process_rxirq_packet(int32_t int_status); //给RTOS用的

...
// 用这样吗？感觉不用
void stmflash_read_byte(uint32_t raddr, uint8_t *pbuf, uint32_t length)
{
    uint32_t i;
		uint32_t temp;
    ...
    {
				printf("%d \r\n ",i);
        temp = stmflash_read_word(raddr);    /* 读取4个字节. */
				pbuf[i+3]     = (temp >> 24) & 0xFF;  /* 保存最高字节到pbuf[i] */
				pbuf[i+2]   = (temp >> 16) & 0xFF;  /* 保存次高字节到pbuf[i+1] */
				pbuf[i+1]   = (temp >> 8) & 0xFF;   /* 保存次低字节到pbuf[i+2] */
				pbuf[i]   = temp & 0xFF;          /* 保存最低字节到pbuf[i+3] */
        raddr += 4; /* 偏移4个字节. */

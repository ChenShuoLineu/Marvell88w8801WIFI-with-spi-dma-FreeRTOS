...
void HAL_DCMI_MspInit(DCMI_HandleTypeDef* hdcmi)
{
    ...
    HAL_GPIO_Init(GPIOE, &gpio_init_struct);            /* 初始化PE5,6引脚 */
		//他们俩不用受RTOS管理 --dcmi还是需要收的吧
    HAL_NVIC_SetPriority(DCMI_IRQn, 7, 0);              /* 抢占优先级0，子优先级5 */
...
{ 
    ...
    else                /* 使用双缓冲 */
    {
        ...
        HAL_NVIC_SetPriority(DMA2_Stream1_IRQn, 3, 0);          /* DMA中断优先级 */
 ...
 * @retval      无
 */
//把一帧图像放进来 那这么来说这个DMA的大小也得合适才行不合适
// 就采集不到全面的
...
{    
    if (__HAL_DMA_GET_FLAG(&g_dma_dcmi_handle, DMA_FLAG_TCIF1_5) != RESET)  /* DMA传输完成 */
    {
        ...
        jpeg_dcmi_rx_callback();     /* 执行摄像头接收回调函数,读取数据等操作在这里面处理 */
    } 
}
